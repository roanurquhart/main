...
It extends the address book system by allowing the user to save favorites, stored internally as a `UniquePersonList`.
Additionally, it implements the following operations:
...
These operations are exposed in the `Model` interface as `Model#addFavorites()` and `Model#getFavoritesList()`.
