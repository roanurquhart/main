...
Format: `addcpny` n/NAME p/NUMBER e/AVG$$ a/ADDRESS s/SECTOR n/#EMPS r/REVENUES [t/TAG]…

Examples:

`addcpny` n/Walmart p/98765432 e/low a/John street, block 123, #01-01  s/Retail n/500

`addcpny` n/Betsy Cakes e/medium a/Bob street, block 232 n p/1234567 s/Bakery n/20 t/popular t/local
...
* Only the name is searched.
* Only full words will be matched e.g. `Han` will not match `Hans`
* Companiess matching at least one keyword will be returned (i.e. `OR` search). e.g. `Hans Bo` will return `Hans Gruber`, `Bo Yang`
****
